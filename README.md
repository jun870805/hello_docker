# **🚀 Ticket Bot Fleet Commander (搶票艦隊中控台)**

這是一個基於 Docker 與 Streamlit 的自動化網頁控制系統，專為高負載搶票任務設計。  
採用 Docker-out-of-Docker (DooD) 架構，允許透過網頁介面動態「召喚」多個獨立的 Chrome 容器，並組成艦隊進行平行任務。
...
├── web\_app.py           \# 核心程式碼 (Streamlit 介面與邏輯)  
...
1. **清理戰場**：移除殘留的 chrome-node 容器 (避免 Network is in use 錯誤)。  
2. **權限修復**：建立 data/ 資料夾並設定權限為 UID 1200 (給 Chrome 使用)。  
3. **啟動服務**：建置並啟動 Docker Compose。

### **2\. 存取中控台**

啟動成功後，請在瀏覽器輸入：

* **http://peterfan0805:8501** (或你的 Server IP)

### **3\. 召喚機器人**

1. 在側邊欄「新增節點 ID」輸入 1 (或其他數字)，點擊 **\[➕ 召喚新機器人\]**。  
2. 等待約 3 秒，下拉選單選擇 **chrome-node-1**。  
3. 輸入網址並點擊 **\[🚀 前往網頁\]**。

### **4\. 監控與接管 (NoVNC)**

網頁會顯示該節點的 NoVNC 連結（預設無密碼）：

* Node 1: http://peterfan0805:7901  
* Node 2: http://peterfan0805:7902  
* Node N: http://peterfan0805:7900+N

## **⚙️ 進階設定說明**

### **關於養號 (Google 登入保持)**

目前程式碼預設為 **無痕模式 (Clean Session)**，適合高強度測試。若需要保留 Google 登入狀態以降低驗證碼機率：

1. 修改 web\_app.py 中的 get\_driver 函式，解開以下註解：  
   \# options.add\_argument("--user-data-dir=/chrome-profile")

2. 修改 web\_app.py 中的 spawn\_new\_node 函式，解開 volumes 掛載 profiles 的註解。  
3. 重啟系統。

### **關於安全限制 (核彈級解法)**

為了防止 Chrome 在新版 Linux Kernel (6.8+) 崩潰，我們在動態生成容器時使用了：

security\_opt=\["seccomp:unconfined"\]

這會解除 Docker 的系統呼叫限制，確保 Chrome 穩定運行，但也降低了容器的隔離性 (在內網環境下可接受)。
